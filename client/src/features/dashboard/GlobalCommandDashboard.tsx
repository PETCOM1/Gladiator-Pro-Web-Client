...
import {
    ...
    CheckCircle2, Globe, Zap, Clock, CreditCard,
    Wrench, UserPlus, MessageSquare, Search, Filter,
    ChevronDown, ChevronUp, X, Check, RefreshCw,
    WifiOff, XCircle, Ban, Unlock
} from 'lucide-react';
...
function OverviewView({ onNav }: { onNav: (v: string) => void }) {
    ...
    return (
        <div>
            <SectionHeader title="System Overview" sub="Real-time snapshot of the Gladiator Pro SaaS platform" />
...
function TenantsView() {
    ...
    return (
        <div>
            <SectionHeader title="Tenant Management" sub="Manage all tenants, plans, and account statuses" />
...
function PaymentsView() {
    ...
    return (
        <div>
            <SectionHeader title="Payment Gateway" sub="Monitor subscriptions, invoices, and billing status" />
...
function MaintenanceView() {
    ...
    const [form, setForm] = useState({ service: '', type: 'corrective' as const, note: '', assignedTo: '' });
    ...
    return (
        <div>
            <SectionHeader title="System Maintenance" sub="Corrective & preventive maintenance logs for the platform" />
...
function OnboardingView() {
    ...
    return (
        <div>
            <SectionHeader title="Onboarding Queue" sub="Review and approve new tenant applications" />
            {pending.length === 0 ? (
...
function TicketsView() {
    ...
    return (
        <div>
            <SectionHeader title="Support Tickets" sub="Tenant complaints and support requests" />
...
// ─── Root Component ────────────────────────────────────────────────────────────
type View = 'overview' | 'tenants' | 'payments' | 'maintenance' | 'onboarding' | 'tickets';

export function GlobalCommandDashboard({ onLogout }: { onLogout: () => void }) {
    const [activeView, setActiveView] = useState<View>('overview');

    const sidebarItems = [
        { icon: <LayoutDashboard size={20} />, label: 'System Overview', active: activeView === 'overview', onClick: () => setActiveView('overview') },
        { icon: <Building2 size={20} />, label: 'Tenant Management', active: activeView === 'tenants', onClick: () => setActiveView('tenants') },
        { icon: <CreditCard size={20} />, label: 'Payment Gateway', active: activeView === 'payments', onClick: () => setActiveView('payments') },
        { icon: <Wrench size={20} />, label: 'System Maintenance', active: activeView === 'maintenance', onClick: () => setActiveView('maintenance') },
        { icon: <UserPlus size={20} />, label: 'Onboarding Queue', active: activeView === 'onboarding', onClick: () => setActiveView('onboarding'), badge: mockOnboarding.filter(o => o.status === 'pending').length || undefined },
        { icon: <MessageSquare size={20} />, label: 'Support Tickets', active: activeView === 'tickets', onClick: () => setActiveView('tickets'), badge: mockTickets.filter(t => t.status === 'open').length || undefined },
    ];
